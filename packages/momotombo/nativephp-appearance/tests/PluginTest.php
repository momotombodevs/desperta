...
/**
 * Plugin validation tests for Appearance.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('momotombo/nativephp-appearance');
        expect($manifest['namespace'])->toBe('Appearance');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath.'/resources/android/AppearanceFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package dev.momotombo.plugins.nativephp_appearance');
        expect($content)->toContain('object AppearanceFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath.'/resources/ios/AppearanceFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum AppearanceFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath.'/src/AppearanceServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Momotombo\NativephpAppearance');
        expect($content)->toContain('class AppearanceServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath.'/src/Facades/Appearance.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Momotombo\NativephpAppearance\Facades');
        expect($content)->toContain('class Appearance extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath.'/src/Appearance.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Momotombo\NativephpAppearance');
        expect($content)->toContain('class Appearance');
    });
});
